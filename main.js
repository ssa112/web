...
readMoreBtns.forEach((btn) => {
  btn.addEventListener("click", function (event) {
    ...
    const closestContainer = currentBtn.closest(".container");

    if (!closestContainer) return;

    const currentText = closestContainer.querySelector(".read-more-text");
    currentText.classList.toggle("read-more-text--show");

    currentBtn.textContent = currentBtn.textContent.includes("Read More")
      ? "Read Less..."
      : "Read More...";
  });
});
